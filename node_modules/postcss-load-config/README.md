...
module.exports = ({ env }) => ({
  ...options
  plugins: {
    'postcss-plugin': env === '***REMOVED***' ? {} : false
  }
})
```
...
module.exports = ({ env }) => ({
  ...options
  plugins: [
    env === '***REMOVED***' ? require('postcss-plugin')() : false
  ]
})
```
...
```json
"scripts": {
  "build": "NODE_ENV=***REMOVED*** node postcss",
...
```json
"scripts": {
  "build": "NODE_ENV=***REMOVED*** webpack",

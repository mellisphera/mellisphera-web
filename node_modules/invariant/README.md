...
A way to provide descriptive errors in development but generic errors in ***REMOVED***.
...
**Note:** When `process.env.NODE_ENV` is not `***REMOVED***`, the message is required. If omitted, `invariant` will throw regardless of the truthiness of the condition. When `process.env.NODE_ENV` is `***REMOVED***`, the message is optional – so they can be minified away.

...
### Node.JS (server-side usage)
  ...
  ```js
  var socket = require('socket.io-client')('***REMOVED***');
  socket.on('connect', function(){});
  socket.on('event', function(data){});
  socket.on('disconnect', function(){});
  ```
...
### IO(url:String, opts:Object):Socket
  ...
  `***REMOVED***users`, a transport connection will be established to
  `***REMOVED***` and a Socket.IO connection will be established to
  `/users`.
